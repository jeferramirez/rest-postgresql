    ...
    constructor(
     ...
     async findAll(): Promise<Asignatura[]> {
      ...
      async updatedAsign(asignatura: any) {
        ...
        try {
          const asigActualizar = await this.asignRepository.findOne(asignatura.id);
          asigActualizar.nombre = asignatura.nombre;
          asigActualizar.numeroCreditos = asigActualizar.numeroCreditos;
          asigActualizar.estado =  asigActualizar.estado;
